...
## Projekt leírás
Sportfogadási arbitrázs (surebetting) detektáló rendszer. Két fogadóiroda (TippMix, IViBet) oddsait scrapeli valós időben, Redis adatbázisban tárolja, és automatikusan kiszámolja az arbitrázs lehetőségeket. Telegram bot-on keresztül értesít.
...
```
Fogadóiroda weboldalak (TippMix, IViBet)
         │
         ▼
┌─────────────────────────────┐
...
2. `tippmix_scrape_multiple.py` + `ivibet_scrape_multiple.py` - Folyamatosan futnak loop-ban
...
- dateparser (dátum parsing, magyar nyelv)
